...
function chatHandler(
  ...
  if (message.includes('.vendb') || message.includes('.vb')) {
    const messaggeSplit = message.split(' ')
    messaggeSplit.shift()
    const result = messaggeSplit.join(' ')

    const finalVendObject = vend(rustplus, mapInfo, rustServerInfo)
        ...
        if (counter < maxLimit) {
          rustplus.sendTeamMessage(
            `(${
              ...
              vending.quantity +
              getSpacebars(vending.quantity.toString().length)
            }:${vending.itemShortName}: for ${
              vending.costPerItem +
              getSpacebars(vending.costPerItem.toString().length)
            }:${vending.currencyShortName}: at ${vending.grid}`
          )
          counter += 1
        }
      })
      discord(client, filteredItems)
    } else {
      rustplus.sendTeamMessage('Misspelled or no offers')
    }
  }

  if (message.includes('.vends') || message.includes('.vs')) {
    const messaggeSplit = message.split(' ')
    messaggeSplit.shift()
    const result = messaggeSplit.join(' ')
